...
const _json = _interopRequireDefault(require('json5'))
...
const rootConfig = _json.default.parse(
  _fs.default.readFileSync(configPath, 'utf-8').toString(),
)
...
while (currentConfig) {
  if (currentConfig.extends) {
    const extendsPath = _path.default.resolve(
      relativeRoot,
      currentConfig.extends,
    )
    const c = _json.default.parse(
      _fs.default.readFileSync(extendsPath, 'utf-8').toString(),
    )
    configs.push(c)
    currentConfig = c
...
const config = [...configs].reverse().reduce(
  (prev, cur) => {
...
if (config.compilerOptions.tsPlusConfig) {
  _fs.default.cpSync(
    config.compilerOptions.tsPlusConfig,
    './tsplus.config.json',
  )
